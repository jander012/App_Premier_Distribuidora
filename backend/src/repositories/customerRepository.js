...
export async function createCustomer({ phone, fullName, cpf, email }) {
  ...
  const { rows } = await query(`SELECT * FROM customers WHERE id = $1`, [result.insertId]);
  return rows[0];
}
...
export async function upsertDefaultAddress(customerId, addr) {
  ...
  const { rows } = await query(`SELECT * FROM customer_addresses WHERE id = $1`, [result.insertId]);
  return rows[0];
}
...
export async function updateAddress(addressId, customerId, addr) {
  ...
  const { rows } = await query(`SELECT * FROM customer_addresses WHERE id = $1 AND customer_id = $2`, [
    addressId,
    customerId,
  ]);
  return rows[0];
...
const defaultAddressSubquery = `(SELECT JSON_OBJECT(
...
export async function listCustomersForAdminByStore(storeId, limit = 100, offset = 0) {
  ...
  const { rows } = await query(
    `SELECT c.*, ${defaultAddressSubquery}
     FROM customers c
     WHERE c.id IN (SELECT DISTINCT customer_id FROM orders WHERE store_id = $1)
     ORDER BY c.updated_at DESC
     LIMIT ${safeLimit} OFFSET ${safeOffset}`,
    [storeId]
  );
